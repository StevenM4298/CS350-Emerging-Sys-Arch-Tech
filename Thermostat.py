...
class TemperatureMachine(StateMachine):
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ##First line of LCD display including current date and time


    
            ## Setup Display Line 2
            if(altCounter < 6):
                ##Second line of LCD display including temperature in Fahrenheit
                temp = floor(self.getFahrenheit())
                lcd_line_2 = f"Temp:{temp} F"
                altCounter = altCounter + 1
            else:
                ##Second line including current state and setpoint
                lcd_line_2 = f"{self.current_state.id} SetPoint:{self.setPoint}"
